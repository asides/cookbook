class ComplexitiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @complexity.save
        format.html { redirect_to complexities_path, notice: 'Complexity was successfully created.' }
        format.json { render action: 'show', status: :created, location: @complexity }
      else
  ...
  def update
    respond_to do |format|
      if @complexity.update(complexity_params)
        format.html { redirect_to complexities_path, notice: 'Complexity was successfully updated.' }
  ...
  private
    ...
    def complexity_params
      params.require(:complexity).permit(:title, :order)
    end
end
